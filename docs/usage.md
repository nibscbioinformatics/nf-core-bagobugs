...
```bash
nextflow run nf-core/bagobugs --input '*_R{1,2}.fastq.gz' -profile docker
```
...
First, go to the [nf-core/bagobugs releases page](https://github.com/nf-core/bagobugs/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
